...
class InformixStreamingChangeEventSource(connectorConfig: InformixConnectorConfig,
  ...
  override def execute(context: ChangeEventSource.ChangeEventSourceContext): Unit = {
    var cdcEngine = offsetContext.getCDCEngine();
    if (cdcEngine == null) {
      cdcEngine = dataConnection.getCDCEngine();
    }
    ...
    var label2TableId: Map[Int, TableId] = null;

    LOGGER.info("================= context  ================= {} ", context);
    LOGGER.info("================= context.isRunning  ================= {} ", context.isRunning);
    LOGGER.info("================= CDC Engine  ================= {} ", cdcEngine);

    while (context.isRunning) {
      if (cdcEngine != null) {
        if (label2TableId == null) {
          label2TableId = cdcEngine.converLabel2TableId()
        }

        //LOGGER.info("================= label2TableId  ================= {} ", label2TableId);
