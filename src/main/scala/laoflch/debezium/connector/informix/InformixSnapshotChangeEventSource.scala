...
/*    if (previousOffset != null && !previousOffset.isSnapshotRunning) {
  ...
  @throws[SQLException]
  @throws[InterruptedException]
  override protected def readTableStructure(sourceContext: ChangeEventSource.ChangeEventSourceContext, snapshotContext: RelationalSnapshotChangeEventSource.RelationalSnapshotContext): Unit = {
    ...
    snapshotContext.offset.asInstanceOf[InformixOffsetContext].getCDCEngine().setWatchTableAndCols(
  ...
  @throws[SQLException]
  override protected def getCreateTableEvent(snapshotContext: RelationalSnapshotChangeEventSource.RelationalSnapshotContext, table: Table) = new SchemaChangeEvent(snapshotContext.offset.getPartition, snapshotContext.offset.getOffset, snapshotContext.catalogName, table.id.schema, null, table, SchemaChangeEventType.CREATE, true)

  override protected def complete(snapshotContext: AbstractSnapshotChangeEventSource.SnapshotContext): Unit = {
    try jdbcConnection.connection.setTransactionIsolation(snapshotContext.asInstanceOf[InformixSnapshotChangeEventSource.InformixSnapshotContext].isolationLevelBeforeStart)
    catch {
      case e: SQLException =>
   ...
   */
  override protected def getSnapshotSelect(snapshotContext: AbstractSnapshotChangeEventSource.SnapshotContext, tableId: TableId): Optional[String] = Optional.of(String.format("SELECT * FROM %s.%s", tableId.schema, tableId.table))
